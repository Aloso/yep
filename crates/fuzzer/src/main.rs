use afl::{__fuzz, fuzz};
use ast::token::{Punctuation, Token, TokenData};
use ast::{Spanned, StringInterner};
...
fn main() {
    fuzz!(|data: &[u8]| {
        if let Ok(text) = std::str::from_utf8(data) {
            ...
            if program.errors().is_empty() && parser::parse(program.tokens()).is_ok() {
                is_balanced(program.tokens()).unwrap();
            }
        }
    });
}
...
fn is_balanced(tokens: &[Spanned<Token>]) -> Result<(), &'static str> {
    let mut open_brackets = OpenBrackets::new();
    let mut eof = false;

    for token in tokens {
        match token.data {
            TokenData::Punct(p) => match p {
                Punctuation::Pipe => {
